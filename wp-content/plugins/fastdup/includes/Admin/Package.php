...
class Package {
	...
	public function add_custom_recurrence( $schedules ) {
		$schedules['njt-fastdup-month'] = array(
			'interval' => 31 * 60 * 60 * 24,
			'display'  => __( 'FastDup Month', 'fastdup' ),
		);
		return $schedules;
	}
	...
	public static function build_event( $package_id ) {
		$package     = self::initial_package_by_id( $package_id );
	...
	public function get_local_package_file( $type_file ) {
		...
		if ( file_exists( $file_path ) ) {
			$url = ( $type_file === 'archive' ) ? $this->store_url . "$file_name" : $file_path;
		}
		return array(
			'file_path' => $file_path,
			'url'       => $url,
