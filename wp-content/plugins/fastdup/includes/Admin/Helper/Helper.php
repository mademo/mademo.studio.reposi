...
class Helper {
	...
	public static function init_archive_directory() {
		...
		if ( ! is_dir( $path_archive_dir ) ) {
			...
			if ( ! file_exists( $path_archive_dir ) . '/index.html' ) {
				@file_put_contents( $path_archive_dir . '/index.html', '' );
			}
			// Restore original web root permission
			@chmod( $path_web_root, $old_web_root_perm );
		} elseif ( ! file_exists( $path_archive_dir ) . '/index.html' ) {
				@file_put_contents( $path_archive_dir . '/index.html', '' );
		}
		...
		if ( ! file_exists( $path_archive_dir_tmp ) ) {
			...
			if ( ! file_exists( $path_archive_dir_tmp ) . '/index.html' ) {
				@file_put_contents( $path_archive_dir_tmp . '/index.html', '' );
			}
		} elseif ( ! file_exists( $path_archive_dir_tmp ) . '/index.html' ) {
				@file_put_contents( $path_archive_dir_tmp . '/index.html', '' );
		}
		...
		if ( ! file_exists( $path_arc_package ) ) {
			...
			if ( ! file_exists( $path_arc_package ) . '/index.html' ) {
				@file_put_contents( $path_arc_package . '/index.html', '' );
			}
		} elseif ( ! file_exists( $path_arc_package ) . '/index.html' ) {
				@file_put_contents( $path_arc_package . '/index.html', '' );
		}
		...
		$needs_update = ! file_exists( $file_name ) || trim( file_get_contents( $file_name ) ) !== trim( $htoutput );
		if ( $needs_update ) {
	...
	public static function ensure_htaccess() {
		$file_name = self::safe_path_slash( NJT_FASTDUP_ARCHIVE_DIR_PATH ) . '/.htaccess';
		$htoutput  = "Options -Indexes\n# Block direct HTTP access to all files\n<IfModule mod_authz_core.c>\n    Require all denied\n</IfModule>\n<IfModule !mod_authz_core.c>\n    Order Allow,Deny\n    Deny from all\n</IfModule>";
		if ( ! file_exists( $file_name ) || trim( file_get_contents( $file_name ) ) !== trim( $htoutput ) ) {
			@file_put_contents( $file_name, $htoutput );
		}
	}
